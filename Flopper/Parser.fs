...
module Parser =
    ...
    // Forward declare the expression parser that will be used in the term parser
            ...
            let! res =
                choice
                    [ attempt ifelse |>> castToStatement
                      ...
                      attempt argument |>> castToStatement
            ...
            let _res =
                choice
                    [ attempt bracketedExpression
                      ...
                      // ... any other expressions that can be operands to an operator
                      ]
    ...
    opParser.AddOperator(
        InfixOperator("=", ws, 15, Associativity.Right, (fun left right -> Tokens.Assign(left, right)))
    )
    ...
    let topLevelParser: Parser<Tokens.Expression, unit> = expression .>> eof

    let parse input =
        match run statementsList input with
